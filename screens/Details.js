...
import {StyleSheet, View, Text, Dimensions, TouchableOpacity, Pressable, Image, FlatList, ScrollView, StatusBar,Button, Modal} from 'react-native';
    ...
    const getValues = async() => {
        await fetch('http://172.104.45.142:8069/get/trip/order/area', {
            ...
            body: JSON.stringify({
                params:{
                    "serial_number": route.params.data,
                    "area_code": route.params.item.area_name,
                }
            })
        })
        .then((res) => res.json())
        ...
        <View style={styles.container}>
            <StatusBar barStyle="auto" />
            {
                ...
                : getvalue && getvalue.clock_in != false ?
                    <View style={[styles.part,{textAlign: 'center',}]}>
                        <Text style={{fontSize: 16, fontWeight: 'bold', color: '#000', textTransform: 'uppercase', textAlign: 'center', borderBottomWidth: 0.5, borderBottomColor: '#000', marginBottom: 10,}}>Clock-in is:</Text>
                        <Text style={{fontSize: 16, fontWeight: 'bold', textAlign: 'justify', color: 'red'}}>{`Your clockin datetime${'\t'}${getvalue.clock_in}at${'\t'}`}</Text><Text style={{textAlign: "center", fontSize: 16, fontWeight: 'bold', color: 'red'}}>{getvalue.area_code}</Text>
                    </View>
                :
                    <View style={[styles.part,{textAlign: 'center',}]}>
                        <Text style={{fontSize: 16, fontWeight: 'bold', color: '#000', textTransform: 'uppercase', textAlign: 'center', borderBottomWidth: 0.5, borderBottomColor: '#000', marginBottom: 10,}}>Clock-in is:</Text>
                        <Text style={{fontSize: 16, fontWeight: 'bold', textAlign: 'justify', color: 'red'}}></Text>
                    </View>
                ...
                
            {
                ...
                (
                    ...
                    onPress={() => {getClockout(); navigation.goBack()}}
                    disabled={false}
                    >
                        <Text style={styles.text}>
                            clck out
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalViewCategorie}>
              <TouchableOpacity onPress={() => setModalVisible(false)}>
                   <Text style={{ textAlign: 'right'}}><IconEvilIcons name="close-o" color={'#000'} size={40} /></Text>
                   <View style={{ alignSelf: 'center', }}>
                                          <Image
                        style={{ width: 150, height: 150, resizeMode: 'contain', borderRadius: 25}}
                        source={image}
                    />
                   </View>
                   <TouchableOpacity 
